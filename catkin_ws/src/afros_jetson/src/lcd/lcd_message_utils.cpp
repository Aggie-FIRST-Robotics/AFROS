...
namespace afros_jetson {
    ...
    boost::array<std::string, 4> strings_from_lcd_set(const lcd_set &message) {
        ...
        for (size_t x = 0; x < lcd_set::_line1_type::size(); ++x) {
            line_char[x] = replace_zeros(message.line1.at(x));
        }
        out.at(0) = std::string{line_char};

        for (size_t x = 0; x < lcd_set::_line2_type::size(); ++x) {
            line_char[x] = replace_zeros(message.line2.at(x));
        }
        out.at(1) = std::string{line_char};

        for (size_t x = 0; x < lcd_set::_line3_type::size(); ++x) {
            line_char[x] = replace_zeros(message.line3.at(x));
        }
        out.at(2) = std::string{line_char};

        for (size_t x = 0; x < lcd_set::_line4_type::size(); ++x) {
            line_char[x] = replace_zeros(message.line4.at(x));
        }
    ...
    void lcd_set_from_strings(lcd_set &message, const boost::array<std::string, 4> &lines, bool backlight_on) {
