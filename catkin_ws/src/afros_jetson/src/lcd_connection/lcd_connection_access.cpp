...
namespace afros_jetson{
    ...
    lcd_publication_access::lcd_publication_access(ros::NodeHandle& node, const std::string& name) : afros_core::publication_access<lcd_data>(node, {name, lcd_con::LCD_PUBLISHER_NAME}, 1){}
    ...
    lcd_subscription_access::lcd_subscription_access(ros::NodeHandle& node, const std::string& name, double frequency, boost::function<void(const lcd_data&)>* callback)
            : afros_core::subscription_access<lcd_data>(node, {name, lcd_con::LCD_SUBSCRIBER_NAME}, 1, frequency, callback){
